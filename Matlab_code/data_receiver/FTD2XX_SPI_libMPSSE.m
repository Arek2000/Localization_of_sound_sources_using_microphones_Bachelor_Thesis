...
% Write to the SPI.
%writebuffer = [25000 25000 25000 25000 25000 25];
writebuffer = [8, 0, 0, 0];
transfer_options = 0; % No chip select used.
%x = calllib(Libname,'SPI_Write',pChannelHandle,writebuffer,length(writebuffer),pNumBytesTransferred,transfer_options);
%{
rdmsgPtr = libpointer('uint8Ptr',[0]); % Same size as msg
msg = [0];

y = calllib(Libname,'SPI_Read',pChannelHandle,rdmsgPtr,length(msg),pNumBytesTransferred,transfer_options);
rdmsg = rdmsgPtr.value;
bytes_transfered = pNumBytesTransferred.value;

rdmsgPtr2 = libpointer('uint8Ptr',[0]); % Same size as msg
y = calllib(Libname,'SPI_Read',pChannelHandle,rdmsgPtr2,length(msg),pNumBytesTransferred,transfer_options);
rdmsg2 = rdmsgPtr2.value;
bytes_transfered = pNumBytesTransferred.value;
%}

% Read:
%{
msg = [8, 0, 0, 0]; % Ex: read command and 2 dummy bytes to recieve data
rdmsgPtr = libpointer('uint8Ptr',[0 0 0 0]); % Same size as msg
transfer_options = 0; % No chip select used.
pNumBytesTransferred = libpointer('uint32Ptr',255);

[ERRnum] = calllib(Libname,'SPI_ReadWrite',pChannelHandle,...
rdmsgPtr,msg,length(msg),pNumBytesTransferred,transfer_options);

bytes_transferred = pNumBytesTransferred.value;
rdmsg = rdmsgPtr.value;
%}

    % channel_info = calllib(Libname,'SPI_GetChannelInfo',pChannelHandle, writebuffer);
    % Clean up
